				...
				if (! isset($localize_strings['jetpack_comments_iframe_src']) || empty($localize_strings['jetpack_comments_iframe_src'])) {
					...
					if (isset($localize_strings['display_comments'])) {
						if ($localize_strings['display_comments']) {
							...
							if ($is_logged_in) {
								$localize_strings['local_comments_commenting_as'] = '<p id="jp-carousel-commenting-as">' . sprintf(
									// Translators: %s is name of current user.	
									__('Commenting as %s', 'catch-gallery'),
								...
								if ($comment_registration) {
									...
									// Translators: %s is a field names which is required.
