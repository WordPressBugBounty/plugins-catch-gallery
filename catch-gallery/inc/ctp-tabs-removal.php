...
if (! function_exists('ctp_switch')) {
	...
	function ctp_switch()
	{
		...
		if (! isset($_POST['security'], $_POST['value'], $_POST['option_name'])) {
			wp_die(esc_html__('Invalid request.', 'catch-gallery'));
		}
		...
		if (! wp_verify_nonce($nonce, 'ctp_tabs_nonce')) {
			wp_die(esc_html__('Unauthorized access!', 'catch-gallery'));
		}

		// Capability check
		if (! current_user_can('manage_options')) {
			wp_die(esc_html__('Permission denied!', 'catch-gallery'));
		}
		...
		if (update_option('ctp_options', $option_value)) {
			...
			esc_html_e('Connection Error. Please try again.', 'catch-gallery');
		}
