...
if (! function_exists('activate_catch_gallery')) :
	function activate_catch_gallery()
	{
		...
		if (is_plugin_active($required)) {
			$message =
				// Translators: Information about pro version is already active while trying to activate free version %2$s is a Pro URL to go to plugin page.
				esc_html__(
					'Sorry, Pro plugin is already active. No need to activate Free version. %1$s&laquo; Return to Plugins%2$s.',
					'catch-gallery'
				);
